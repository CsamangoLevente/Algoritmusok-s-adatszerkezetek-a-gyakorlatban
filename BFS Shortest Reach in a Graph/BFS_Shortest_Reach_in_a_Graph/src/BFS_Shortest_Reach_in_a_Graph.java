...
Utána   megszorozzuk 6-tal.
        ...
        while (q-- > 0) {
            ...
            int[] dist = bfs(graf, n, start); //A bfs fügvény meghívása
    ...
    private static int[] bfs(List<List<Integer>> graf, int n, int start) {
        ...
        while (!sor.isEmpty()) { //Amíg a sor nem üres
            int csucs = sor.poll(); // kiveszünk egy csúcsot

            for (int next : graf.get(csucs)) {
                if (tavolsag[next] == -1) { // ha még nem láttuk
                    tavolsag[next] = tavolsag[csucs] + 6; // távolsága: szülő + 6
                    sor.add(next); // betesszük a sorba
                }
            }
        }
